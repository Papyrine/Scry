...
/// </summary>
public sealed class PneumaticClient(Func<QueryRequest, CancellationToken, Task<QueryResponse>> transport)
{
    /// <summary>Creates a client that POSTs queries to an HTTP endpoint.</summary>
    public static PneumaticClient ForHttp(HttpClient http, string endpoint) =>
    ...
    public IQueryable<T> Source<T>(string name) =>
        new PneumaticQueryable<T>(new PneumaticQueryProvider(this, name));
    ...
    {
        var json = PneumaticJson.Serialize(request);
        ...
        {
            throw new PneumaticRequestException((int)message.StatusCode, body);
        }

        return PneumaticJson.DeserializeResponse(body);
    }
}

/// <summary>Thrown when the server rejects or fails a query.</summary>
public sealed class PneumaticRequestException(int statusCode, string body) :
    Exception($"Pneumatic query failed ({statusCode}): {body}")
{
