...
sealed class QueryTranslator
{
    ...
    QueryOp TranslateCall(MethodCallExpression call)
    {
        switch (call.Method.Name)
        {
            ...
            default:
                throw new NotSupportedException($"LINQ operator '{call.Method.Name}' is not supported by Pneumatic.");
        }
    }
    ...
    static NotSupportedException Unsupported(Expression expression) =>
        new($"Expression '{expression.NodeType}' is not supported by Pneumatic.");
