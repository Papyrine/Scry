...
/// </summary>
sealed class QueryExecutor(SkrySchema schema, SkryOptions options)
{
    ...
    public QueryResponse Execute(QueryRequest request, DbContext db, IServiceProvider services)
    {
        ...
        return QueryResponse.Create(ResultKind.List, JsonSerializer.SerializeToElement(array, SkryJson.Options));
    }
    ...
    IQueryable ApplyPolicy(IQueryable query, SkrySource source, DbContext db, IServiceProvider services)
    {
        ...
        var context = new SkryPolicyContext(services, db);
    ...
    static QueryResponse Scalar<T>(T value) =>
        QueryResponse.Create(ResultKind.Scalar, JsonSerializer.SerializeToElement(value, SkryJson.Options));

    static QueryResponse Single(object[]? row, ProjectionPlan plan)
    {
        var payload = row is null
            ? JsonSerializer.SerializeToElement<object?>(null)
            : JsonSerializer.SerializeToElement(Shape(row, plan), SkryJson.Options);
