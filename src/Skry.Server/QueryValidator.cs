...
/// </summary>
sealed class QueryValidator(SkrySchema schema, SkryOptions options)
{
    public SkrySource Validate(QueryRequest request)
    {
    ...
    SkryMember ResolvePath(IReadOnlyList<string> path, Type rootType, bool requireScalar, string what)
    {
        if (path.Count == 0)
        {
    ...
    static SkryValidationException Reject(string message) =>
        new(message);
}
