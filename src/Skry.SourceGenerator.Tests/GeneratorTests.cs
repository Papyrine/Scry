...
namespace Skry.SourceGenerator.Tests;
...
public class GeneratorTests
{
    ...
    static Task VerifyGenerated(string modelSource)
    {
        ...
        var dllPath = Path.Combine(Path.GetTempPath(), $"SkryModel_{Guid.NewGuid():N}.dll");
        try
        {
            ...
            var generator = new SkryGenerator();
            GeneratorDriver driver = CSharpGeneratorDriver.Create(
                generators: [generator.AsSourceGenerator()],
                additionalTexts: null,
                parseOptions: null,
                optionsProvider: new TestOptionsProvider(new() { ["build_property.SkryModelDll"] = dllPath }));
