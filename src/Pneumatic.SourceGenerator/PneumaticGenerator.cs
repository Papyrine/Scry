...
/// <summary>
/// Reads the server model assembly (pointed at by the <c>PneumaticModelDll</c> build property) and
...
/// </summary>
[Generator]
public class PneumaticGenerator :
    IIncrementalGenerator
{
    const string generatedNamespace = "Pneumatic.Generated";

    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var path = context.AnalyzerConfigOptionsProvider
            .Select((provider, _) => GetProperty(provider, "build_property.PneumaticModelDll"));
        ...
        var stamp = context.AnalyzerConfigOptionsProvider
            .Select((provider, _) => GetProperty(provider, "build_property.PneumaticModelStamp"));
    ...
    static readonly DiagnosticDescriptor readFailed = new(
        "PNEU001",
        "Failed to read the Pneumatic model assembly",
        "{0}",
        "Pneumatic",
    ...
    static void Emit(SourceProductionContext context, ModelExtract extract)
    {
        ...
        {
            context.AddSource("PneumaticEnums.g.cs", EmitEnums(extract.Enums));
        }

        context.AddSource("PneumaticQuery.g.cs", EmitQuery(extract.Sources));
    }
    ...
    static string EmitQuery(EquatableArray<SourceInfo> sources)
    {
        ...
        builder.AppendLine("public sealed class PneumaticQuery");
        builder.AppendLine("{");
        builder.AppendLine("    readonly global::Pneumatic.Client.PneumaticClient client;");
        builder.AppendLine();
        builder.AppendLine("    public PneumaticQuery(global::Pneumatic.Client.PneumaticClient client) => this.client = client;");
