...
public class WireSerializationTests
{
    ...
    public void UnknownDiscriminatorFailsClosed()
    {
        ...
        Assert.Throws<PneumaticWireException>(() => PneumaticJson.DeserializeRequest(json));
    }

    [Test]
    public void MalformedJsonFailsClosed() =>
        Assert.Throws<PneumaticWireException>(() => PneumaticJson.DeserializeRequest("{ not json"));

    static Task VerifyRoundTrip(QueryRequest request)
    {
        var json = PneumaticJson.Serialize(request);
        var roundTripped = PneumaticJson.DeserializeRequest(json);
        var reserialized = PneumaticJson.Serialize(roundTripped);
