...
sealed partial class QueryTranslator
{
    ...
    static object? Evaluate(Expression expression)
    {
        ...
        // Read directly where the shape allows — see ClosureReader for which shapes those are and
        // why. Every other shape is compiled, which on the interpreter the browser runs is the cost
        // that reader exists to skip.
        if (ClosureReader.TryRead(expression, out var value))
        {
            return value;
        }
