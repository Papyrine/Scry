...
/// from the wire.
/// </summary>
sealed class ExpressionBuilder(PneumaticSchema schema)
{
    ...
    public ProjectionPlan BuildDefaultProjection(Type elementType)
    {
        ...
        {
            throw new PneumaticValidationException($"Type '{elementType.Name}' is not queryable.");
        }
    ...
    public ProjectionPlan BuildGroupProjection(Projection projection, Type elementType, Type keyType)
    {
        ...
        foreach (var member in projection.Members)
        {
            ...
            var leaf = expression switch
            {
                ...
                _ => throw new PneumaticValidationException("Unsupported grouped projection member.")
            };
        ...
        List<IReadOnlyList<string>> shape)
    {
        foreach (var member in projection.Members)
        {
            ...
            switch (member.Value)
            {
                ...
                default:
                    throw new PneumaticValidationException("Unsupported projection member.");
            }
        }
    }
    ...
    Expression Build(Expr expr, ParameterExpression parameter, Type? expected) =>
        expr switch
        {
            ...
            _ => throw new PneumaticValidationException($"Unsupported expression '{expr.GetType().Name}'.")
        };
    ...
    Expression BuildMemberAccess(Expression root, IReadOnlyList<string> path)
    {
        ...
        foreach (var segment in path)
        {
            ...
            {
                throw new PneumaticValidationException(
    ...
    Expression BuildBinary(BinaryExpr binary, ParameterExpression parameter)
    {
        ...
        return binary.Op switch
        {
            ...
            _ => throw new PneumaticValidationException($"Unsupported binary operator '{binary.Op}'.")
        };
    }
    ...
    Expression BuildUnary(UnaryExpr unary, ParameterExpression parameter)
    {
        ...
        return unary.Op switch
        {
            ...
            _ => throw new PneumaticValidationException($"Unsupported unary operator '{unary.Op}'.")
        };
    }
    ...
    Expression BuildCall(CallExpr call, ParameterExpression parameter)
    {
        ...
        return call.Function switch
        {
            ...
            _ => throw new PneumaticValidationException($"Unsupported function '{call.Function}'.")
        };
    }
    ...
    Expression BuildAggregate(AggregateExpr aggregate, ParameterExpression group, Type elementType)
    {
        ...
        {
            throw new PneumaticValidationException($"Aggregate '{aggregate.Function}' requires a member selector.");
        }
        ...
        return aggregate.Function switch
        {
            ...
            _ => throw new PneumaticValidationException($"Unsupported aggregate '{aggregate.Function}'.")
        };
    }
    ...
    static MethodInfo StringMethod(string name, params Type[] parameters) =>
        ...
        throw new PneumaticValidationException($"string.{name} not found.");
