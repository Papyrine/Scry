...
/// soft-delete, row security). Register via <see cref="PneumaticOptions.AddPolicy{TEntity,TPolicy}"/>
/// or the <c>[ReturnableWith]</c> attribute.
/// </summary>
public interface IReturnablePolicy<T>
{
    IQueryable<T> Filter(IQueryable<T> source, PneumaticPolicyContext context);
}

/// <summary>Context handed to an <see cref="IReturnablePolicy{T}"/>, exposing the request services.</summary>
public sealed class PneumaticPolicyContext(IServiceProvider services, DbContext db)
{
