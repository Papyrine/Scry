...
/// </summary>
public static class PneumaticJson
{
    ...
    static T Deserialize<T>(string json, string what)
    {
        try
        {
            return JsonSerializer.Deserialize<T>(json, Options) ??
                   throw new PneumaticWireException($"Query {what} deserialized to null.");
        }
        catch (JsonException exception)
        {
            throw new PneumaticWireException($"Invalid query {what}: {exception.Message}", exception);
        }
    }
}
