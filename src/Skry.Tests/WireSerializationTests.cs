...
public class WireSerializationTests
{
    ...
    public void UnknownDiscriminatorFailsClosed()
    {
        ...
        Assert.Throws<SkryWireException>(() => SkryJson.DeserializeRequest(json));
    }

    [Test]
    public void MalformedJsonFailsClosed() =>
        Assert.Throws<SkryWireException>(() => SkryJson.DeserializeRequest("{ not json"));

    static Task VerifyRoundTrip(QueryRequest request)
    {
        var json = SkryJson.Serialize(request);
        var roundTripped = SkryJson.DeserializeRequest(json);
        var reserialized = SkryJson.Serialize(roundTripped);
