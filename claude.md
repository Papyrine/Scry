...
- **Until a stable 1.0.0, wire breaks are allowed.** Nothing is deployed against this yet, so a change that would strand an older client or server is a normal design choice rather than something to work around: pick the right shape, don't add negotiation, fallbacks, or version shims to preserve a contract no one depends on. Still say plainly in the PR/commit that a break happened, and keep it a deliberate call rather than a side effect. This paragraph goes away at 1.0.0 — after that, the paragraph above governs.
