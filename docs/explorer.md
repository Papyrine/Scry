...
`csv` writes them as displayed, with RFC 4180 quoting and a UTF-8 BOM so Excel reads non-ASCII values correctly. A cell a spreadsheet would read as a formula — one beginning with `=`, `+`, `-`, `@`, a tab, or a carriage return — is prefixed with an apostrophe, which the spreadsheet takes as text and does not display; the rows are database content, and a value an end user typed is not something a spreadsheet should execute. A number keeps its sign. It is offered only for a **flat** result: [projecting into a navigation](wire-format.md) nests an object inside each row, and a grid cannot hold that without flattening the shape away. `xml` (a `row` element each, a child element per member) and `json` (the rows exactly as the server sent them) keep the nesting, so they are offered for every result the table can render.
  ...
  Types: [
    ...
    },
    {
      ...
      Members: [
        ...
        {
          Name: Workstation,
          TypeDisplay: WorkstationQueryModel?,
          NeedsNullDefault: false,
          IsNavigation: true,
          IsCollection: false,
          IsAttachment: false,
...
4. `SnippetExecutor` compiles the same snippet against the models — compiled once for the schema, so a run compiles only the snippet itself — runs it against a capturing client to build the LINQ expression tree, and calls the production `ToScryRequest` — so the wire request shown is produced by exactly the same translation the real client performs.
...
The host page is served under a `Content-Security-Policy`. Every source is the explorer's own origin; the allowances past that are the ones the page cannot run without — `'wasm-unsafe-eval'` for the .NET runtime, inline styles and `blob:` workers for Monaco — and the page's own two inline scripts, which are allowed by hash. The page is built once per mapping — base href written in, scripts hashed from that same text, bytes tagged — so the hashes are of the page as served: a nonce would have to be minted into the page per response, and every revalidation would then be a download. `connect-src` is the origin alone unless `QueryEndpoint` names another origin, which is then added. `frame-ancestors 'self'` keeps the page out of another origin's frame. Only the document carries the policy — the assets it loads are governed by the page's. The mount sets the header rather than appending to it, so a policy written earlier in the app's own pipeline is replaced. A policy added *after* it, by a reverse proxy say, is a second `Content-Security-Policy` header, and a browser enforces both: the effective policy is their intersection, so one that does not name the page's script hashes stops the explorer booting however permissive this one is.
