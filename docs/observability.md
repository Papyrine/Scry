...
- **`Error` is the real message.** For a `Failed` outcome the client saw a generic 500; the entry carries the underlying exception's message. The audit trail is where execution failures are readable. Two things follow. The text is the provider's — SQL Server's, say — so an auditor forwarding entries somewhere that must not see table names treats it as it treats `Request`. And a client can produce a `Failed` entry at will: a division by a constant of its choosing, or a conversion of text that is not a number, fails at execution rather than validation, so the failure count is a client-drivable signal in the way the rejection count is, and an alert on it wants a threshold rather than a trigger.
