...
```
<sup><a href='/src/Scry.Tests/TestModel.cs#L523-L531' title='Snippet source file'>snippet source</a> | <a href='#snippet-returnablePolicy' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->
...
A cold scope decides every row once, serialized per scope so a burst of requests pays it once rather than each. That cost is the size of the table, and it is paid **per scope key**: on a per-user scope, every caller the store has not seen reads and decides the whole table on their first query, and a thousand new callers pay it a thousand times. Every allowed key then travels to the database with each query — bound as a single parameter, not written into the SQL — which suits an allow-list bounded per caller and not one that grows with the table. Two options bound the two costs. `options.MaxCachedPolicyRows` bounds the work: a refresh under it counts the undecided rows before reading them, and a scope past it is refused from the count rather than materialized. `options.MaxCachedPolicyKeys` bounds the result, turning an allow-list that quietly grew unbounded into a message rather than a slow query. Neither is set by default.
