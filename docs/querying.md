...
`Cast` is **not** the same operator spelled differently, and is not supported. `OfType` filters — a row that is not the derived type is left out. `Cast` asserts — every row is required to be that type already, and one that is not is an error. EF does carry that assertion, but as a materialization fault: the query runs, returns every row, and throws on the first that does not match. So whether it succeeds depends on which rows exist rather than on what the query says, which puts it past the point where Scry can answer — [validation decides on the request alone](security.md), before anything is executed, and this would reach the database and come back a generic `500`. `OfType` says the useful half and is decidable up front. An upcast to a base is a no-op here, since the wire names a source rather than a type and projects by member name.
